...
import article.controlers.Pagination;
...
//		}
	...
	public List<ArticleVO> getArticleList() throws Exception {
		...
		try {
			...
			//다음 레코드 존재하는지 확인 -> 존재하면 레코드를 담을 ArticleVO 인스턴스 생성하고 담음
			while(rs.next()) {
	...
	/*	Connection cn = null;
		...
		try {
			...
			if (ps.executeUpdate() == 0) {
			 ...
			 if문을 쓰지않고 그냥 ps.excuteUpdate()만 쓴다면
			   바밀번호가 틀려도 no rows일 뿐(조건에 해당하지 않아서), 
			   정상 수행되어서 글 수정 성공이 되어버림. 그래서 if문으로 exception으로 던져줌.
...
*/
	...
	@Override
	public List<ArticleVO> getArticleList(Pagination pagination) throws Exception {

		SqlMapClient sqlMap = QueryHandler.getInstance();
		return sqlMap.queryForList("article.getArticleList", pagination);

		/*
		 * Connection cn =null; PreparedStatement ps = null; ResultSet rs =
		 * null;
		 * 
		 * List<ArticleVO> list = new ArrayList<ArticleVO>(); StringBuffer sql =
		 * new StringBuffer(); sql.append(" SELECT B.*");
		 * sql.append(" FROM (SELECT rownum AS rnum, A.*");
		 * sql.append("       FROM (SELECT no, title, name, regdate, viewcount"
		 * ); sql.append("             FROM tb_article");
		 * sql.append("             ORDER BY no DESC) A) B");
		 * sql.append(" WHERE rnum BETWEEN ? and ?");
		 * 
		 * try { cn = getConnection(); ps = cn.prepareStatement(sql.toString());
		 * ps.setLong(1, pagination.getStartnum()); ps.setLong(2,
		 * pagination.getEndnum()); rs = ps.executeQuery(); while(rs.next()) {
		 * ArticleVO articleVO = new ArticleVO();
		 * articleVO.setNo(rs.getLong("no"));
		 * articleVO.setTitle(rs.getString("title"));
		 * articleVO.setName(rs.getString("name"));
		 * articleVO.setRegdate(rs.getDate("regdate"));
		 * articleVO.setViewcount(rs.getInt("viewcount")); list.add(articleVO);
		 * } } finally { dbClose(rs, ps, cn); } return list; }
		 */
	}
}
